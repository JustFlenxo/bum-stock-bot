...
def make_family_embeds(products: dict):
    # Build multiple embeds kept under Discord's 6000-char limit.
    ...
    for fam_name in ["Dum Bum", "Zom Bum", "Viper", "Original", "Other"]:
        items = families[fam_name]
        if not items:
            continue
        ...
        part = 1
        for ln in lines:
            if len(chunk) + len(ln) + 1 > 3500:  # safe buffer
                e = discord.Embed(
                    title=f"🧨 {fam_name} Firecrackers" + (f" (part {part})" if part > 1 else ""),
                    description=chunk,
                    color=0x1abc9c,
                    timestamp=now_utc
                )
    ...
    header = discord.Embed(
        ...
        description=f"Auto-updates every 10 minutes • Last update: **{now_utc.strftime('%Y-%m-%d %H:%M UTC')}**\n"
                    "Each section below is a separate embed to avoid Discord size limits.",
        color=0x1abc9c,
        timestamp=now_utc
    )
    header.set_footer(text="Only firecrackers/petards • Links + prices")
...
async def check_stock():
    try:
        ...
        embeds = make_family_embeds(current)

        # Option A: ONE message, MANY embeds
        await status_msg.edit(embeds=embeds)
