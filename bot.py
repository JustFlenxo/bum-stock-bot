...
def parse_products_from_html(html: str, page_url: str):
    ...
    for block in soup.select("div.product-block"):
        ...
        products[title] = {
            "stock": stock, "link": link, "brand": guess_brand(title),
            "price": extract_price(block), "family": fam,
        }
    return products
...
def scrape_all_firecrackers():
    ...
    for title, info in merged.items():
        info["nec"] = extract_nec_from_product_page(info["link"])
        time.sleep(0.15)
    return merged
...
def build_grouped_embed(products: dict) -> discord.Embed:
    ...
    def format_line(name, info):
        ...
        return f"{emoji} **[{name}]({link})** — _{brand}_ — **{price}** — **NEC {nec}**"
    ...
    embed.set_footer(text="Only firecrackers/petards • Links + prices + NEC")
    return embed
