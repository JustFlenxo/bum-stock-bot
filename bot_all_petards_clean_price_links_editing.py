...
def extract_price(block: BeautifulSoup) -> str:
    # Try common price selectors on Ekopyro
    price_el = (
...
def parse_products_from_html(html: str, page_url: str):
    ...
    for block in soup.select("div.product-block"):
        ...
        products[title] = {"stock": stock, "link": link, "brand": brand, "price": price}
    return products
...
def build_status_embed(products: dict) -> discord.Embed:
    ...
    embed.set_footer(text="Updated every 10 minutes • Prices shown")
    return embed
