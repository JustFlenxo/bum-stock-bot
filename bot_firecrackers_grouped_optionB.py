...
def build_family_embeds(fam_name: str, items):
    # Build up to 10 embeds for a single family.
    # If there are too many lines, the last embed will mention how many are truncated.
    now_utc = datetime.now(timezone.utc)

    # sort: in-stock first, then A-Z
    ...
    chunk = ""
    part = 1
    max_embeds = 9  # we'll add a header embed later, total max 10
    for ln in lines:
    ...
    if shown_lines < total_lines and embeds:
        ...
        last = embeds[-1]
        last.description += f"\n…and **{remaining}** more items (truncated to fit Discord limits)."
