...
# ================= CONFIG =================
# Search multiple keywords to catch all petards/firecrackers.
SEARCH_URLS = [
...
# Words that usually mean it's NOT a petard/firecracker (exclude)
BAD_WORDS = [
...
# Words that usually mean it IS a petard/firecracker (include)
GOOD_WORDS = [
...
def parse_products_from_html(html: str, page_url: str):
    ...
    for block in soup.select("div.product-block"):
        ...
        products[title] = {"stock": stock, "link": link, "brand": brand}
    return products
...
def build_status_embed(products: dict) -> discord.Embed:
    ...
    embed = discord.Embed(
        title="🧨 ALL PETARDS / FIRECRACKERS — Live Stock",
        description="Auto-updates every 10 minutes. Only petards/firecrackers (no rockets/cakes).",
        color=0x3498db,
    ...
    embed.set_footer(text="Ekopyro monitor • links included • brands shown")
    return embed
